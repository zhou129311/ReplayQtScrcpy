...
void AdbProcess::initSignals()
{
    ...
    connect(this, &QProcess::errorOccurred, this, [this](QProcess::ProcessError error) {
        if (QProcess::FailedToStart == error) {
            ...
            qCritical() << err.toStdString().c_str() << error;
        }
    });
...
void AdbProcess::syncExecute(const QString &serial, const QStringList &args)
{
    m_standardOutput = "";
    m_errorOutput = "";
    QStringList adbArgs;
...
void AdbProcess::pushReplayFile(const QString &serial, const QString &path)
{
    QStringList adbArgs;
    adbArgs << "push"
            << path
            << "/data/local/tmp/replay";
    execute(serial, adbArgs);
}

void AdbProcess::pullReplayFile(const QString &serial, const QString &path)
{
    QStringList adbArgs;
    adbArgs << "pull"
            << "/data/local/tmp/replay"
...
void AdbProcess::pullImageFile(const QString &serial, const QString &name, const QString &dstPath)
{
    QStringList adbArgs;
    adbArgs << "pull"
            << "/data/local/tmp/screenshot/"+name
            << dstPath;
    execute(serial, adbArgs);
}

void AdbProcess::rmScreenshotDir(const QString &serial)
{
    QStringList adbArgs;
    adbArgs << "rm"
            << "-rf"
...
void AdbProcess::homeKeyCode(const QString &serial)
{
    QStringList adbArgs;
    adbArgs << "input"
            << "keyevent"
...
void AdbProcess::forceStop(const QString &serial, const QString &pkgName)
{
    QStringList adbArgs;
    adbArgs << "am"
            << "force-stop"
...
//QString AdbProcess::getDeviceName(const QString &serial)
//{
//    QString manu = "ro.product.manufacturer";
