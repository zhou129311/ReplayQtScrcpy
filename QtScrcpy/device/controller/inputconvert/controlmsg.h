...
public:
    enum ControlMsgType
    {
        ...
        CMT_RECORD_REPLAY = 11, //弃用了，改为在PC端录制脚本
        CMT_REPLAY, //弃用了，改为在PC端录制脚本
        CMT_SLEEP_TIME, //弃用了，改为在PC端录制脚本
        CMT_SLIDER_REPLAY, //弃用了，改为在PC端录制脚本
    };
    ...
    void setReplayRecord(bool record);
    void setReplayState(int state, qint32 count);
    void setSleepTime(qint32 time);
    void setSliderPos(qint32 pos);

    bool needWriteReplayMsg();

    QString jsonControlMsgData();
