...
Device::Device(DeviceParams params, QObject *parent) : QObject(parent), m_params(params)
{
    ...
    connect(&m_adb, &AdbProcess::adbProcessResult, this, [this](AdbProcess::ADB_EXEC_RESULT processResult) {
        if (AdbProcess::AER_SUCCESS_START != processResult) {
            ...
            if (!rel.isEmpty() && m_params.deviceName.isEmpty()) {
                QStringList adbArgs;
                adbArgs << "shell"
                        << "getprop"
                        << "ro.product.model";
                m_adb.execute(m_params.serial, adbArgs);
                m_params.deviceName = rel;
            } else if (!rel.isEmpty() && !name_end){
                name_end = true;
                m_params.deviceName = m_params.deviceName + "_" + rel;
                QStringList adbArgs;
                adbArgs << "shell"
                        << "wm"
                        << "size";
                m_adb.execute(m_params.serial, adbArgs);
            } else if (!rel.isEmpty()&& !wh_end) {
                wh_end = true;
                QStringList list = rel.split(":");
                if (list.size() > 1) {
                    QString size = list.at(1);
                    size = size.trimmed();
                    list = size.split("x");
                    m_params.deviceWidth = list.first().toInt();
                    m_params.deviceHeight = list.last().toInt();
                    qInfo() << "device size :" << m_params.deviceWidth << "," << m_params.deviceHeight;
                }
                QStringList adbArgs;
                adbArgs << "shell"
                        << "pm" << "resolve-activity" << "-a" << "android.intent.action.MAIN" << "-c" << "android.intent.category.HOME"
                        << "|" << "grep" << "packageName" << "|" << "head" << "-n" << "1";
                m_adb.execute(m_params.serial, adbArgs);
            } else if (!rel.isEmpty() && !luancher_end) {
                luancher_end = true;
                QStringList list = rel.split("=");
                if (list.size() == 2) {
                    QString name = list.at(1);
                    m_params.launcherPkg = name;
                    qInfo() << "launcher packageName :" << name;
                }
                QStringList adbArgs;
                adbArgs << "shell"
                        << "getprop"
                ...
                m_params.sdkVersion = rel.toInt();
                qInfo() << "sdk version :" << rel;
            }
        }
    });
    QStringList adbArgs;
...
{
    return m_params.deviceName;
}

const QString &Device::getLauncherPkg()
{
    return m_params.launcherPkg;
}

const int &Device::getSdkVersion()
{
    return m_params.sdkVersion;
}

const int &Device::getWidth()
{
    return m_params.deviceWidth;
}

const int &Device::getHeight()
{
    return m_params.deviceHeight;
}
...
void Device::onScreenshot(QString path)
{
    if (!m_vb) {
        return;
    }
    ...
    m_vb->lock();
    // screenshot
    saveFrame(path, m_vb->peekRenderedFrame());
    m_vb->unLock();
}
...
void Device::initSignals()
{
    ...
    if (m_controller) {
        ...
        connect(this, &Device::replayRecord, m_controller, &Controller::onReplayRecord);
        connect(this, &Device::replayState, m_controller, &Controller::onReplayState);
        connect(this, &Device::insertSleepTime, m_controller, &Controller::onInsertSleepTime);
        connect(this, &Device::replaySliderPos, m_controller, &Controller::onReplaySlider);
    }
    if (m_videoForm) {
...
void Device::updateReplayCount(int count)
{
    if (!m_videoForm) {
        return;
    }
    m_videoForm->showReplayCount(count);
}

void Device::replayEnd()
{
    if (!m_videoForm) {
        return;
    }
    m_videoForm->replayEnd();
}

void Device::replayPause()
{
    if (!m_videoForm) {
        return;
    }
    m_videoForm->replayPause();
}

void Device::replayResume()
{
    if (!m_videoForm) {
        return;
    }
    m_videoForm->replayResume();
}

bool Device::saveFrame(QString savePath, const AVFrame *frame)
{
    if (!frame) {
        return false;
    }
