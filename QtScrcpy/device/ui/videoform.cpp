...
void VideoForm::setController(QPointer<Controller> controller)
{
    m_controller = controller;
    if (!m_replayForm) {
        return;
    }
    m_replayForm->setController(m_controller);
}

void VideoForm::showReplayCount(int count)
{
    if (!m_replayForm) {
        return;
    }
    m_replayForm->showReplayCount(count);
}

void VideoForm::replayEnd()
{
    if (!m_replayForm) {
        return;
    }
    m_replayForm->replayEnd();
}

void VideoForm::replayPause()
{
    if (!m_replayForm) {
        return;
    }
    m_replayForm->replayPause();
}

void VideoForm::replayResume()
{
    if (!m_replayForm) {
        return;
    }
    m_replayForm->replayResume();
}
...
void VideoForm::showReplayForm(bool show)
{
    if (!m_replayForm) {
        m_replayForm = new ReplayForm(this, ReplayForm::AP_OUTSIDE_LEFT);
        m_replayForm->setDevice(m_device);
        m_replayForm->setController(m_controller);
    }
    if (m_toolForm) {
        m_replayForm->move(pos().x() - m_replayForm->width(), pos().y());
    } else {
        m_replayForm->move(pos().x() + geometry().width(), pos().y());
    }
    m_replayForm->setVisible(show);
}
...
void VideoForm::staysOnTop(bool top)
{
    ...
    if (m_replayForm) {
        m_replayForm->setWindowFlag(Qt::WindowStaysOnTopHint, top);
    }
    if (needShow) {
        show();
    }
...
void VideoForm::mousePressEvent(QMouseEvent *event)
{
    ...
    // 判断videoWidget显示区域是否包含鼠标点击的坐标
    if (m_videoWidget->geometry().contains(event->pos())) {
        // device指针是否为空，为空说明device断开连接
        if (!m_device) {
            return;
        }

        // 把鼠标在PC桌面的坐标映射成在videoWidget显示区域的坐标
...
//            QString posTip = QString(R"("mouseLeftPress": {"x": %1, "y": %2})").arg(x).arg(y);
