...
#define COMMON_REPLAY_KEY "RePlayPath"
#define COMMON_REPLAY_DEF ""

#define COMMON_REPLAY_COUNT_KEY "RePlayCount"
#define COMMON_REPLAY_COUNT_DEF "1"

#define COMMON_REPLAY_FILE_KEY "RePlayFile"
#define COMMON_REPLAY_FILE_DEF ""
...
QString Config::getRePlayPath()
{
    QString replay;
    m_userData->beginGroup(GROUP_COMMON);
    replay = m_userData->value(COMMON_REPLAY_KEY, COMMON_REPLAY_DEF).toString();
    m_userData->endGroup();
    return replay;
}

void Config::setRePlayPath(const QString &path)
{
    m_userData->beginGroup(GROUP_COMMON);
    m_userData->setValue(COMMON_REPLAY_KEY, path);
    m_userData->endGroup();
}

QString Config::getRePlayCount()
{
    QString replay;
    m_userData->beginGroup(GROUP_COMMON);
    replay = m_userData->value(COMMON_REPLAY_COUNT_KEY, COMMON_REPLAY_COUNT_DEF).toString();
    m_userData->endGroup();
    return replay;
}

void Config::setRePlayCount(const QString &count)
{
    m_userData->beginGroup(GROUP_COMMON);
    m_userData->setValue(COMMON_REPLAY_COUNT_KEY, count.toInt());
    m_userData->endGroup();
}


QString Config::getRePlayFilePath()
{
    QString replay;
    m_userData->beginGroup(GROUP_COMMON);
    replay = m_userData->value(COMMON_REPLAY_FILE_KEY, COMMON_REPLAY_FILE_DEF).toString();
    m_userData->endGroup();
    return replay;
}

void Config::setRePlayFilePath(const QString &path)
{
    m_userData->beginGroup(GROUP_COMMON);
    m_userData->setValue(COMMON_REPLAY_FILE_KEY, path);
    m_userData->endGroup();
}
